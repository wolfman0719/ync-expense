...
export const App = () => {
  ...
  // 削除後にリストから該当IDの項目を除外するコールバック
  const onDeleteItem = useCallback((deletedId: any) => {
    setExpenseItemList((prev: any[]) => prev.filter((eitem: any) => eitem.id !== deletedId));
    setResponse("");
  }, []);
  ...
  return (
    <>
      ...
      <div id="expensecontent" style={{ width: "50%", height: `${height * 0.9}px`, overflow: "auto", border: "solid #000000 1px" }}>
        <ExpenseItem response={response} onDeleteItem={onDeleteItem} />
      </div>
    </>
  );
}
